...
export function createProjectApi(ipcRenderer: IpcRendererLike): DesktopApiSlice<ProjectApiKeys> {
  return {
    ...
    getProject: (projectId) =>
      ipcRenderer.invoke(IPC_CHANNELS.project.get, projectId) as ReturnType<DesktopApi['getProject']>,
    updateProjectPrompt: (projectId, aiPrompt) =>
      ipcRenderer.invoke(IPC_CHANNELS.project.updatePrompt, projectId, aiPrompt) as ReturnType<DesktopApi['updateProjectPrompt']>,
    ...
    getProjectFiles: (projectId) =>
      ipcRenderer.invoke(IPC_CHANNELS.project.getFiles, projectId) as ReturnType<DesktopApi['getProjectFiles']>,
    getFile: (fileId) => ipcRenderer.invoke(IPC_CHANNELS.file.get, fileId) as ReturnType<DesktopApi['getFile']>,
    getFilePreview: (filePath) =>
      ipcRenderer.invoke(IPC_CHANNELS.file.getPreview, filePath) as ReturnType<DesktopApi['getFilePreview']>,
    deleteFile: (fileId) =>
    ...
    addFileToProject: (projectId, filePath, options) =>
      ipcRenderer.invoke(IPC_CHANNELS.project.addFile, projectId, filePath, options) as ReturnType<DesktopApi['addFileToProject']>,
    getSegments: (fileId, offset, limit) =>
      ipcRenderer.invoke(IPC_CHANNELS.file.getSegments, fileId, offset, limit) as ReturnType<DesktopApi['getSegments']>,
    ...
    updateSegment: (segmentId, targetTokens, status) =>
      ipcRenderer.invoke(IPC_CHANNELS.segment.update, segmentId, targetTokens, status) as ReturnType<DesktopApi['updateSegment']>
  };
}
