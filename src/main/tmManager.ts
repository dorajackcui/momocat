...
export class TMManager {
  ...
  public set(source: string, target: string) {
    ...
    const stmt = this.db.prepare('INSERT OR REPLACE INTO translations (source, target) VALUES (?, ?)')
  ...
  public getBatch(sources: string[]): Record<string, string> {
    ...
    for (let i = 0; i < sources.length; i += CHUNK_SIZE) {
      ...
      const stmt = this.db.prepare(`SELECT source, target FROM translations WHERE source IN (${placeholders})`)
  ...
  public importFromExcel(filePath: string): number {
    try {
      ...
      const insert = this.db.prepare('INSERT OR REPLACE INTO translations (source, target) VALUES (?, ?)')
  ...
  public fuzzySearch(query: string, threshold = 70, limit = 5): TMMatch[] {
    ...
    try {
      ...
      candidates = ftsStmt.all(`"${escapedQuery}"`, ftsLimit) as { source: string, target: string }[]
    } catch (e) {
