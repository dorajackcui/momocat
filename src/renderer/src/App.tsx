...
function App(): JSX.Element {
  ...
  const handleColumnConfirm = async (sourceIndex: number, targetIndex: number) => {
    ...
    let tmMatches: Record<string, string> = {}
    const CHUNK_SIZE = 1000
    
    // Initial render without matches first (for immediate feedback)
    let processedSegments = validRows.map((row, index) => ({
      id: index,
      ...
      try {
        ...
        try {
          ...
          if (msg.includes('EPERM') || msg.includes('access')) {
             alert('Note: System permission denied (Sandbox). File has been downloaded via browser instead. (Styles may not be preserved in fallback mode)')
          }
        } catch (fallbackError) {
  ...
  return (
    <div style={{ 
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        {/* Main Editor Area */}
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: '20px', overflowY: 'auto', minHeight: 0 }}>
          {filePath && <div style={{ marginBottom: '15px', color: '#666', fontSize: '12px' }}>File: {filePath}</div>}
          ...
          {segments.length > 0 && (
            <div style={{ 
              ...
              <div style={{ fontWeight: 'bold', backgroundColor: '#fafafa', padding: '10px', borderBottom: '1px solid #d9d9d9' }}>Source Text</div>
              <div style={{ fontWeight: 'bold', backgroundColor: '#fafafa', padding: '10px', borderBottom: '1px solid #d9d9d9' }}>Target Text</div>
          ...
          {segments.length > 0 && (
            <div style={{ marginTop: '20px', color: '#999', fontSize: '12px', textAlign: 'center' }}>
